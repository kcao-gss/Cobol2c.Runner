...
.PARAMETER Headless    When set, session bootstrap uses Enter-ConsoleSession (console park via
                        tscon) instead of Connect-TA01Rdp / mstsc. No RDP viewer required.
                        Default (unset) = current attended behavior unchanged.
#>
param(
    [string]$Suite,
    [string]$Machine,
...
function Invoke-SuiteRun ([string]$Base, [string]$SuiteName, [string]$BatName,
                           ...
                           [string]$RecoveryEnabled = 'true', [switch]$Headless) {
    $maxRecoveries  = 2
    $recoveryCount  = 0
    $currentTests   = $Tests
    $isFirstAttempt = $true
    ...
    while ($true) {
        ...
        $hardDeadline    = (Get-Date).AddMinutes([Math]::Max(90, $currentTests.Count * 12))
        ...
        if ($wedged) {
            ...
            if (($RecoveryEnabled -eq 'true') -and $recoveryCount -lt $maxRecoveries) {
                ...
                Invoke-VMRecovery -Machine $Machine -Ta01Pw $Ta01Pw -TaskName $taskName -Headless:$Headless
                $recoveryCount++
...
if ($MyInvocation.InvocationName -ne '.') {
    ...
    $failRun = Invoke-SuiteRun -Base $FailBase -SuiteName $Suite    -BatName $failBat `
                                ...
                                -RecoveryEnabled $autoRecover -Headless:$Headless
